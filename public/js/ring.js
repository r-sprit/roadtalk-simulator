...
function senddata() {
    if (itime % 65 == 0) {
        ...
        xmlhttp.open("POST", "/updateVehcileData");
        xmlhttp.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
        var road_vehicles = mainroad.getRoadVehcilesJson();
        xmlhttp.send(JSON.stringify(road_vehicles));

        road_vehicles.forEach(function (road_vehicle) {
            var vehcile_container = document.getElementById("connectedVehiclesList");
            vehcile_container.innerHTML = vehcile_container.innerHTML +
                '<span>' + road_vehicle.car_id + "</span><br />";
